...
from .focus_capsules import FocusCaps
from .layers import *

class UCaps28(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=False):
        super(UCaps28, self).__init__()
        ...
        self.conv1 = UDenseBlock(32, 8, 4, upsample=False) # formerly 2 layers each dense op
        # self.caps1 = UCapsuleLayer(4, 16, "conv", k=3, s=2, t_1=4, z_1=16, routing=3, padding=1)
        self.caps1 = FocusCaps(4, 16, 4, 16, "conv", kernel_size=3, Nh=4, stride=2, padding=1, c2p=False, mode='att')
        self.conv2 = UDenseBlock(64, 16, 4, upsample=False)
        # self.caps2 = UCapsuleLayer(8, 16, "conv", k=3, s=2, t_1=8, z_1=16, routing=3, padding=1)
        self.caps2 = FocusCaps(8, 16, 8, 16, "conv", kernel_size=3, Nh=4, stride=2, padding=1, c2p=False, mode='att')
        self.conv3 = UDenseBlock(128, 32, 4, upsample=False)
        # self.caps3 = UCapsuleLayer(8, 32, "conv", k=3, s=2, t_1=8, z_1=32, routing=3, padding=1)
        self.caps3 = FocusCaps(8, 32, 8, 32, "conv", kernel_size=3, Nh=4, stride=2, padding=1, c2p=False, mode='att')
        self.conv4 = UDenseBlock(256, 64, 4, upsample=False)
        # self.caps4 = UCapsuleLayer(16, 32, "conv", k=3, s=2, t_1=16, z_1=32, routing=3, padding=1)
        self.caps4 = FocusCaps(16, 32, 16, 32, "conv", kernel_size=3, Nh=4, stride=2, padding=1, c2p=False, mode='att')
        self.conv5 = UDenseBlock(512, 128, 4, upsample=False)

        # Up ops
        # self.upcaps_1 = UCapsuleLayer(32, 32, "deconv", k=3, s=2, t_1=16, z_1=32, routing=3, padding=1)
        self.upcaps_1 = FocusCaps(32, 32, 16, 32, "deconv", kernel_size=3, Nh=4, stride=2, padding=1, c2p=False, mode='att')
        self.upconv_1 = UDenseBlock(1024, 128, 4, upsample=True)
        # self.upcaps_2 = UCapsuleLayer(16, 32, "deconv", k=3, s=2, t_1=8, z_1=32, routing=3, padding=1)
        self.upcaps_2 = FocusCaps(16, 32, 8, 32, "deconv", kernel_size=3, Nh=4, stride=2, padding=1, c2p=False, mode='att')
        self.upconv_2 = UDenseBlock(512, 64, 4, upsample=True)
        # self.upcaps_3 = UCapsuleLayer(8, 32, "deconv", k=3, s=2, t_1=8, z_1=16, routing=3, padding=1)
        self.upcaps_3 = FocusCaps(8, 32, 8, 16, "deconv", kernel_size=3, Nh=4, stride=2, padding=1, c2p=False, mode='att')
        self.upconv_3 = UDenseBlock(256, 32, 4, upsample=True)
        # self.upcaps_4 = UCapsuleLayer(8, 16, "deconv", k=3, s=2, t_1=4, z_1=16, routing=3, padding=1)
        self.upcaps_4 = FocusCaps(8, 16, 4, 16, "deconv", kernel_size=3, Nh=4, stride=2, padding=1, c2p=False, mode='att')
        self.upconv_4 = UDenseBlock(128, 16, 4, upsample=True)
    ...
    def forward(self, x):
        ...
        x10.squeeze_(1)
        xcat = torch.cat((x10, x7), 1)
        x11 = self.upconv_1(xcat)
        ...
        x12.squeeze_(1)
        xcat = torch.cat((x12, x5), 1)
        x13 = self.upconv_2(xcat)
        ...
        x14.squeeze_(1)
        xcat = torch.cat((x14, x3), 1)
        x15 = self.upconv_3(xcat)
        ...
        x16.squeeze_(1)
        xcat = torch.cat((x16, x1), 1)
        x17 = self.upconv_4(xcat)
